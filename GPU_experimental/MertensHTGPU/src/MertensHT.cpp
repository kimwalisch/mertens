...
#ifdef USE_GPU
    // GPU bruteDoubleSum (MHT_GPU_BRUTE / OVERLAP). Per-m 64/128 split, exactly
    // as MertensHurst's S2 and HT's own brute do: x/m decreases with m, so route
    // the majority with x/m < cap to the GPU (passing the precomputed quotient),
    // and keep the few small-m tasks (x/m >= cap) on the CPU's 64/128-bit path.
    // cap = 2^60 is the float divider's exact range; the n-values must be < 2^32.
    if (!flag && (gpu::useBrute() || gpu::useOverlap()) && gpu::numDevices() > 0
        && lBp <= (1L << 32)) {
        static const Int128 kGpuDivCap = []{
            const char* e = std::getenv("MHT_GPU_DIVCAP");   // test override (bits)
            return e ? ((Int128)1 << atoi(e)) : ((Int128)1 << 60);
        }();
        long nnz = (long)nzN.size();
        long mGpuLo = (long)(x / kGpuDivCap) + 1;            // first m with x/m < cap
        if (mGpuLo < lA)  mGpuLo = lA;
        if (mGpuLo > lAp) mGpuLo = lAp;

        // Small-m tail (x/m >= cap): CPU's existing per-m 64/128-bit brute.
        if (mGpuLo > lA)
            S += bruteDoubleSum(lA, mGpuLo, muA, nzN.data(), nzG.data(), nnz, x);

        // 64-bit-eligible majority: GPU. CPU precomputes lxm = floor(x/m).
        long gpuCount = lAp - mGpuLo;
        if (gpuCount > 0) {
            std::vector<uint64_t> lxm(gpuCount);
            for (long i = 0; i < gpuCount; i++)
                lxm[i] = (uint64_t)(x / (Int128)(mGpuLo + i));
            std::vector<uint32_t> nzN32(nnz);
            for (long i = 0; i < nnz; i++) nzN32[i] = (uint32_t)nzN[i];
            std::vector<int64_t> part(gpuCount);
            gpu::bruteMap((const signed char*)(muA + (mGpuLo - lA)), lxm.data(),
                          gpuCount, nzN32.data(), (const signed char*)nzG.data(),
                          nnz, part.data());
            for (long i = 0; i < gpuCount; i++) S += (Int128)part[i];
        }
        return S;
    }
#endif
...
#pragma omp parallel for schedule(dynamic)
    ...
    START_PROFILE();
#ifdef USE_GPU
    // GPU offload (MHT_GPU_FACSUM): compute every FacToSumMu value on the GPU,
    // then form the per-task prefix sums on the CPU (cheap). 64-bit only.
    if (use64 && gpu::useFacsum() && gpu::numDevices() > 0) {
        // Output buffer: a shared (zero-copy) buffer when available, else heap.
        static int64_t* gpuVal = nullptr;
        static long gpuValCap = 0;
        static bool gpuValShared = false;
        if (gpuValCap < bigDelta) {
            if (gpuVal) { if (gpuValShared) gpu::sharedFree(gpuVal); else free(gpuVal); }
            gpuVal = (int64_t*)gpu::sharedAlloc(bigDelta * sizeof(int64_t));
            gpuValShared = (gpuVal != nullptr);
            if (!gpuVal) gpuVal = (int64_t*)malloc(bigDelta * sizeof(int64_t));
            gpuValCap = bigDelta;
        }
        gpu::facsumMap(fun, (const uint64_t*)sqfprod, (uint64_t)lx, r0, bigDelta,
                       gpuVal);
#pragma omp parallel for schedule(static)
        for (long j = 0; j < ntasks; j++) {
            long base = j * (Delta + 1);
            long s = 0;
            for (long t = 0; t <= Delta; t++) { s += (long)gpuVal[base + t]; S[base + t] = s; }
        }
    } else
#endif
#pragma omp parallel for schedule(dynamic)
...
// ============================================================================
// LargeNonFree (pipelined) — GPU FacToSumMu(block k) overlapped with CPU
// fillFactBlock(block k+1). Selected by MHT_GPU_PIPELINE; equivalent in value
// to largeNonFree() below (validate: same M(x)). 64-bit only.
// ============================================================================
#ifdef USE_GPU
static Int128 largeNonFreePipelined(Int128 x, long v, long u, int ntasks, bool profile, Int64 sarrCapBytes) {
    timeval tStart, tEnd;
    double tSieve = 0, tFill = 0, tGpu = 0;
    ...
    int64_t* val = (int64_t*)malloc(bigDelta * sizeof(int64_t));
    if (!isprime || !mup || !S || !sarrFun || !sarrSqfprod || !sarrOff || !val) {
        std::fprintf(stderr, "largeNonFreePipelined: allocation failed\n");
        std::abort();
    }
    ...
    ulong lx = (ulong)x;  // caller guarantees use64

    auto fillBlock = [&](long rr0) {
#pragma omp parallel for schedule(dynamic)
        for (long j = 0; j < ntasks; j++)
            fillFactBlock(sarrFun + j * (Delta + 1), sarrSqfprod + j * (Delta + 1),
                          isprime, rr0 + j * (Delta + 1), Delta + 1,
                          threadTabla[omp_get_thread_num()]);
    };
    // val[] (from the GPU) -> per-task prefix sums S[] + global offsets.
    auto finishBlock = [&](Int128 S0) {
#pragma omp parallel for schedule(static)
        for (long j = 0; j < ntasks; j++) {
            long base = j * (Delta + 1);
            long s = 0;
            for (long t = 0; t <= Delta; t++) { s += (long)val[base + t]; S[base + t] = s; }
        }
        sarrOff[0] = S0;
        for (long j = 1; j < ntasks; j++)
            sarrOff[j] = sarrOff[j - 1] + S[j * (Delta + 1) - 1];
        sarrOff[ntasks] = sarrOff[ntasks - 1] + S[bigDelta - 1];
    };
    ...
    ulong lxLnf = lx;
    long qCur = (long)(lxLnf / (ulong)u);
    long qPrev = (long)(lxLnf / (ulong)(u + 1));
    long qEst;
    long n = u;

    if (profile) getDayTime(tStart);
    fillBlock(curR0);                                   // block 0 (CPU)
    if (profile) { getDayTime(tEnd); tFill += getDuration(tStart, tEnd); }

    bool more = true;
    while (more) {
        // Launch current block's FacToSumMu on the GPU (copies sarrFun/sqf).
        if (profile) getDayTime(tStart);
        gpu::facsumDispatch(sarrFun, (const uint64_t*)sarrSqfprod, lx, curR0, bigDelta);
        if (profile) { getDayTime(tEnd); tGpu += getDuration(tStart, tEnd); }

        long nThreshCur = (long)(x / ((Int128)curR0 + bigDelta));
        bool needNext = (nThreshCur > v);

        // Overlap: CPU fills the next block while the GPU computes this one.
        if (needNext) {
            if (profile) getDayTime(tStart);
            fillBlock(curR0 + bigDelta);
            if (profile) { getDayTime(tEnd); tFill += getDuration(tStart, tEnd); }
        }
        ...
        if (profile) { getDayTime(tEnd); tGpu += getDuration(tStart, tEnd); }
        finishBlock(S0chain);
        S0chain = sarrOff[ntasks];

        // Consume this block's n-range: n down to nThreshCur (or v on last block).
        long blockLow = needNext ? nThreshCur : v;
        for (; n > blockLow; n--) {
            if (n < n0) {
                n0 = n0 - (Delta + 1);
                if (n0 < 1) n0 = 1;
                ...
                if (profile) { getDayTime(tEnd); tSieve += getDuration(tStart, tEnd); }
            }
            ...
            if (muN) {
                ...
                sig += mu_quot;
                const long idx = xn - curR0;
                Sum += muN * mu_quot
                     + 2 * muN * (-sig + (sarrOff[idx / (Delta + 1)] + (Int128)S[idx]));
            }
        }

        if (!needNext) more = false;
        else          curR0 += bigDelta;   // next block already filled into sarrFun
    }

    if (profile) {
        printf("  LargeNonFree (pipelined) breakdown:\n");
        printf("        fillFactBlock(CPU): %10.6f\n", tFill);
        printf("        FacToSumMu(GPU+wait): %10.6f\n", tGpu);
        printf("        fillMuBlock: %10.6f\n", tSieve);
        printf("        (fill/GPU overlap, so wall < sum)\n");
    }

    for (int t = 0; t < nthreads; t++) free(threadTabla[t]);
    delete[] threadTabla;
    free(val); free(sarrFun); free(sarrSqfprod); free(sarrOff);
...
static Int128 largeNonFree(Int128 x, long v, long u, int ntasks, bool profile, Int64 sarrCapBytes) {
#ifdef USE_GPU
    // 64-bit only (the GPU FacToSumMu path); fall back to CPU otherwise.
    if (gpu::usePipeline() && gpu::numDevices() > 0 && ((uI)x <= (uI)(ulong)-1))
        return largeNonFreePipelined(x, v, u, ntasks, profile, sarrCapBytes);
#endif
    timeval tStart, tEnd;
    ...
    // For the GPU FacToSumMu path, allocate fun/sqfprod as engine-owned shared
    // buffers (zero-copy on a single unified GPU); otherwise plain heap.
    bool sarrShared = false;
    PrimFact* sarrFun = nullptr;
    ulong* sarrSqfprod = nullptr;
#ifdef USE_GPU
    if (gpu::useFacsum() && ((uI)x <= (uI)(ulong)-1) && gpu::numDevices() > 0) {
        sarrFun = (PrimFact*)gpu::sharedAlloc(bigDelta * sizeof(PrimFact));
        sarrSqfprod = (ulong*)gpu::sharedAlloc(bigDelta * sizeof(ulong));
        sarrShared = (sarrFun && sarrSqfprod);
        if (!sarrShared) {
            if (sarrFun) { gpu::sharedFree(sarrFun); sarrFun = nullptr; }
            if (sarrSqfprod) { gpu::sharedFree(sarrSqfprod); sarrSqfprod = nullptr; }
        }
    }
#endif
    if (!sarrFun)     sarrFun     = (PrimFact*)malloc(bigDelta * sizeof(PrimFact));
    if (!sarrSqfprod) sarrSqfprod = (ulong*)malloc(bigDelta * sizeof(ulong));
    Int128* sarrOff = (Int128*)malloc((ntasks + 1) * sizeof(Int128));
...
#ifdef USE_GPU
    if (sarrShared) { gpu::sharedFree(sarrFun); gpu::sharedFree(sarrSqfprod); }
    else            { free(sarrFun); free(sarrSqfprod); }
#else
    free(sarrFun); free(sarrSqfprod);
#endif
    free(sarrOff);
    free(isprime); free(mup); free(S);
    return Sum;
...
#ifdef USE_GPU
    // Overlap mode: LargeFree and LargeNonFree are independent, so run LargeFree
    // (which offloads its brute double-sums to the GPU) on a helper thread while
    // LargeNonFree drives the CPU. The GPU brute work is hidden under the CPU
    // bottleneck — additive capacity, so it wins even when the GPU is slower
    // per-op. Standalone mode: don't combine with FACSUM/PIPELINE (both would
    // drive the GPU from two threads).
    if (gpu::useOverlap() && gpu::numDevices() > 0 && ((uI)x <= (uI)(ulong)-1)) {
        getDayTime(t0);
        Int128 Bu = bruteM(u);
        getDayTime(t1);
        ...
        if (profile) {
            printf("------------- Profile (OVERLAP) --------------\n");
            printf("              BruteM: %10.6f\n", getDuration(t0, t1));
            printf("  LargeFree(GPU) || LargeNonFree(CPU): %10.6f\n", getDuration(t1, t3));
            printf("               Total: %10.6f\n", getDuration(t0, t3));
            printf("----------------------------------------------\n");
        }
