...
#ifdef USE_GPU
    // GPU plumbing self-test: enumerate devices and validate a kernel vs CPU.
    if (std::strcmp(argv[1], "--gpu-selftest") == 0) {
        gpu::printDevices();
        long N = (argc > 2) ? std::atol(argv[2]) : (1L << 22);
        return gpu::selftest(N) ? 0 : 1;
    }
#endif
